...
if ! az vm show --resource-group $resourceGroupName --name DC 1> /dev/null 2> /dev/null; then
    ...
    (
        ...
        az vm create --resource-group $resourceGroupName --name DC --image MicrosoftWindowsServer:WindowsServer:2022-datacenter-azure-edition:latest --admin-username $adminUserName --admin-password $adminPassword --license-type Windows_Server \
            --nics VM-DC-NIC --os-disk-size-gb 128 --os-disk-caching ReadWrite --os-disk-name "VM-DC-OSDisk" --size Standard_B2s --storage-sku StandardSSD_LRS 1> /dev/null
...
if ! az vm show --resource-group $resourceGroupName --name SQL 1> /dev/null 2> /dev/null; then
    ...
    (
        ...
        az vm create --resource-group $resourceGroupName --name SQL --image MicrosoftSQLServer:sql2019-ws2019:SQLDEV:latest --admin-username "local-${adminUserName}" --admin-password $adminPassword \
